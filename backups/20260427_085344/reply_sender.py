...
# PATCH_TOPIC_ROUTING_FACT_ONLY_SAFE
# Adds Telegram forum topic routing without touching env, db schema, google_io, ai_router, credentials, sessions.
_ORIG_send_reply_ex = send_reply_ex
_ORIG_send_reply = send_reply
...
def send_reply_ex(chat_id: str, text: str, reply_to_message_id: Optional[int] = None, message_thread_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    payload = {"chat_id": str(chat_id), "text": text, "disable_web_page_preview": True}
    try:
        if message_thread_id is not None and int(message_thread_id) > 0:
    ...
    try:
        ...
        if r.status_code == 200 and r.json().get("ok") is True:
            bot_message_id = r.json().get("result", {}).get("message_id")
            logger.info("reply_ok chat_id=%s thread=%s reply_to=%s chars=%s bot_message_id=%s", chat_id, message_thread_id, reply_to_message_id, len(text), bot_message_id)
            return {"ok": True, "bot_message_id": bot_message_id}
        logger.error("reply_fail code=%s body=%s", r.status_code, r.text[:500])
        return {"ok": False, "bot_message_id": None}
    except Exception as e:
...
def send_reply(chat_id: str, text: str, reply_to_message_id: Optional[int] = None, message_thread_id: Optional[int] = None) -> bool:
    return bool(send_reply_ex(chat_id=chat_id, text=text, reply_to_message_id=reply_to_message_id, message_thread_id=message_thread_id).get("ok"))
