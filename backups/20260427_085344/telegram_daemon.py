...
# === CANON_PASS6_SINGLE_INSTANCE_LOCK ===
import fcntl as _cp6_fcntl
import sys as _cp6_sys
import os as _cp6_os
_CP6_LOCK_PATH = "/tmp/areal_telegram_daemon.lock"
try:
    _cp6_lock_fp = open(_CP6_LOCK_PATH, "w")
    _cp6_fcntl.flock(_cp6_lock_fp.fileno(), _cp6_fcntl.LOCK_EX | _cp6_fcntl.LOCK_NB)
    _cp6_lock_fp.write(str(_cp6_os.getpid()))
    _cp6_lock_fp.flush()
except (BlockingIOError, OSError):
    print("CANON_PASS6: telegram_daemon already running, exit", flush=True)
    _cp6_sys.exit(0)
# === END_CANON_PASS6_SINGLE_INSTANCE_LOCK ===
...
SHORT_CONFIRM = ["да", "нет", "ок", "подтверждаю", "не так", "ага", "верно", "доволен", "да доволен", "доволен результатом", "согласен", "принято", "принял"]
NEGATIVE_CONFIRM = ["нет", "не так"]
FINISH_PHRASES = [
    "спасибо поиск завершен", "поиск завершен",
    "не надо", "можно завершать",
    "задача закрыта", "закрывай", "хватит", "завершая", "завершаем", "завершить", "заверши", "доволен", "доволен результатом", "задачу можно завершать", "да доволен"]
CANCEL_PHRASES = [
...
async def _handle_control_text(message, tg_id: int, text: str, lower: str, reply_to: int | None, topic_id: int = 0) -> bool:
    ...
    _lower_norm_pre = lower.strip().rstrip('.!?,').strip()
    _confirm_pre = _lower_norm_pre in SHORT_CONFIRM or any(p in _lower_norm_pre for p in ('доволен', 'завершай', 'завершая', 'завершить', 'да доволен'))
    if _confirm_pre and not parent and _lower_norm_pre not in NEGATIVE_CONFIRM:
        closed = await close_latest_open_task(tg_id, 'confirmed:DONE')
        await message.answer('Задача завершена' if closed else 'Нет активных задач')
        return True
    if not parent:
        return False
    ...
    if parent_state in ("AWAITING_CONFIRMATION", "IN_PROGRESS"):
        _lower_norm = lower.strip().rstrip(".!?,").strip()
        _confirm_hit = _lower_norm in SHORT_CONFIRM or any(p in _lower_norm for p in ("доволен", "завершай", "завершая", "завершить", "можно завершать", "да", "готово", "закрывай"))
        if _confirm_hit and _lower_norm not in NEGATIVE_CONFIRM:
            now = now_iso()
            async with aiosqlite.connect(DB) as db:
                await db.execute("UPDATE tasks SET state='DONE', updated_at=? WHERE id=?", (now, parent_id))
...
async def universal_handler(message: types.Message):
    ...
    try:
        ...
        if message.photo:
            ...
            try:
                ...
                if isinstance(drive_result, dict) and drive_result.get("ok") and drive_result.get("drive_file_id"):
                    ...
                    await create_task(message, "drive_file", json.dumps(payload, ensure_ascii=False), "NEW")
                    await message.reply("Получил фото, обрабатываю...")
                    return
                else:
        ...
        if reply_to:
            ...
            if parent:
                ...
                if _has_any_phrase(lower, FINISH_PHRASES):
                    async with aiosqlite.connect(DB) as db:
                        await db.execute("UPDATE tasks SET state='DONE', updated_at=? WHERE id=?", (now_iso(), parent_id))
                        await db.execute("INSERT INTO task_history (task_id, action, created_at) VALUES (?,?,?)", (parent_id, "finish:DONE", now_iso()))
                        await db.execute("UPDATE pin SET state='CLOSED', updated_at=? WHERE task_id=? AND state='ACTIVE'", (now_iso(), parent_id))
                        await db.commit()
                    await message.answer("Задача закрыта")
                    return
                async with aiosqlite.connect(DB) as db:
                    cur = await db.execute("SELECT result FROM tasks WHERE id=?", (parent_id,))
                    prow = await cur.fetchone()
                    presult = prow[0] if prow else ""
                if parent_state == "AWAITING_CONFIRMATION" and presult and "drive.google.com" in (presult or ""):
                    await message.answer("Файл уже есть в чате. Напиши: готово / уточни / отмена")
                    return
                if parent_state == "WAITING_CLARIFICATION":
                    async with aiosqlite.connect(DB) as db:
        ...
        if message.voice:
            ...
            await download_telegram_file(tg_file.file_path, local_path)
            # === CANON_RECOVERY_VOICE_NO_DRIVE ===
            # Voice files are runtime-only: Telegram voice -> local temp -> STT -> text task -> delete.
            # Do NOT upload voice_*.ogg to Google Drive and do NOT create drive_file tasks from voice.
            logger.info("VOICE_NO_DRIVE local=%s", local_path)
            # === END_CANON_RECOVERY_VOICE_NO_DRIVE ===
            ...
            _VOICE_CONTROL_HARD = ["отбой", "отмена", "не надо", "всё", "все", "готово", "можно закрывать", "задача закрыта", "закрывай", "принято", "принял", "доволен", "да доволен", "доволен результатом", "завершай", "заверши", "завершить", "завершая", "можно завершать", "задачу можно завершать"]
            _voice_norm = voice_lower.strip().rstrip(".!?,").strip()
            _voice_hit = _voice_norm in _VOICE_CONTROL_HARD or any(p in _voice_norm for p in ("доволен", "завершай", "завершая", "завершить", "можно завершать", "задача закрыта", "закрывай", "готово"))
            if _voice_hit:
...
# DISABLED_PASS13             if voice_lower.strip() in CHAT_ONLY_PHRASES:
# DISABLED_PASS13                 await message.answer(CHAT_ONLY_PHRASES[voice_lower.strip()])
                return
            await create_task(message, "text", "[VOICE] " + voice_text, "NEW")
            try:
                os.remove(local_path)
                logger.info("VOICE_TEMP_DELETED local=%s", local_path)
            except Exception as _del_err:
                logger.warning("VOICE_TEMP_DELETE_FAIL local=%s err=%s", local_path, _del_err)
            return
        ...
        logger.exception("INTAKE_CREATE_TASK_FAILED: %s", e)
        try:
            await message.answer("INTAKE_CREATE_TASK_FAILED: Ошибка при постановке голосовой задачи")
        except:
            pass
...
if __name__ == "__main__":
    asyncio.run(main())
# === CANON_PASS# ===
# OVERLAY_APPLIED: logger.exception for INTAKE_CREATE_TASK_FAILED

# === CANON_PASS13_CHAT_ONLY_PHRASES_FIX ===
if 'CHAT_ONLY_PHRASES' not in globals():
    CHAT_ONLY_PHRASES = {
        "ок": "Принял.",
        "ага": "Принял.",
        "бредишь?": "Понял, уточни задачу.",
        "понял": "Принял.",
