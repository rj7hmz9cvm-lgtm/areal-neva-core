...
async def process_one_task():
    ...
    if input_type == "voice":
        ...
        voice_echo_text = f"Распознано: {transcript.strip()}" 
    ...
    if final_state == "AWAITING_CONFIRMATION":
        ...
        if input_type == "voice" and voice_echo_text:
            final_text = voice_echo_text + "\n\n" + ai_result
        final_text = ai_result
        if input_type == "voice" and voice_echo_text:
            final_text = voice_echo_text + "\n\n" + ai_result
        msg_id = await send_reply(BOT_TOKEN, int(chat_id), final_text + "\n\nПодтвердить выполнение?", reply_to)
        if not msg_id:
            db = await get_db()
