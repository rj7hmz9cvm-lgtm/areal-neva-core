...
async def get_next_ready() -> dict | None:
    # FIX: лочим сразу в AWAITING_CONFIRMATION — это валидное состояние в schema CHECK.
    # DELIVERING убран: его нет в CHECK constraint, любая попытка записать его
    # даёт sqlite3.IntegrityError. _running_ready set в воркере предотвращает
    # двойную обработку без промежуточного состояния.
    return await asyncio.to_thread(_fetch_and_lock, "RESULT_READY", "AWAITING_CONFIRMATION")
