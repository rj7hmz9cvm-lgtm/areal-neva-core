...
def _load_archive_context(chat_id, topic_id):
    import sqlite3
    archive_context = ''
    try:
        ...
        cursor.execute("""
            ...
            WHERE chat_id = ? AND key LIKE 'archive_legacy_%'
            ORDER BY timestamp DESC LIMIT 10
        """, (chat_id,))
        rows = cursor.fetchall()
        for row in rows:
            archive_context += row["value"] + "\n"
        conn.close()
    except Exception as e:
        archive_context = f"Error loading archive context: {e}"
    return archive_context

def _load_memory_context(chat_id, topic_id):
    import sqlite3
    memory_context = ''
    try:
        conn = sqlite3.connect("/root/.areal-neva-core/data/memory.db")
        ...
        """, (chat_id, f"topic_{topic_id}_%"))
        rows = cursor.fetchall()
        for row in rows:
            memory_context += row["value"] + "\n"
        conn.close()
    except Exception as e:
        memory_context = f"Error loading memory context: {e}"
    return memory_context

def process_task(task):
    topic_id = task.get('topic_id', 0)
    memory_context = _load_memory_context(task['chat_id'], topic_id)
    if not memory_context:
        archive_context = _load_archive_context(task['chat_id'], topic_id)
        if archive_context:
            task['archive_context'] = archive_context
        else:
            task['archive_context'] = 'No relevant context found in archive'
    else:
