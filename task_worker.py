...
async def _run_intake_bg(task: dict) -> None:
    ...
    try:
        await _notify(
            chat_id,
            f"✅ Принято\nЗапрос: {_preview('голосовое сообщение' if input_type == 'voice' else raw_input)}",
            topic_id,
        )
        ...
        current = await get_task(task_id)
        if not current or _status(current) != "INTAKE":
            return
        ...
        if source == "automation_daemon" and input_type in ("automation", "followup"):
            ...
            if ok:
                await _safe_transition(task_id, "RESULT_READY", "task_worker")
            return
        ...
        if input_type == "voice":
            ...
            # === AREAL WORKER BUS START ===
            import memory_engine, file_registry, document_engine, table_engine, Path
            _cid = locals().get('chat_id', 0)
            _raw = str(locals().get('raw_input') or locals().get('text') or locals().get('message') or '')
            _fp = str(locals().get('file_path') or locals().get('path') or locals().get('media_path') or '')
            if not _fp:
                for k, v in locals().items():
                    if isinstance(v, str) and v.startswith('/root/') and '.' in v: _fp = v; break
            if _fp and Path(_fp).exists():
                try: file_registry.index_file(_cid, _fp)
                except: pass
                _ft = table_engine.read(_fp) if Path(_fp).suffix.lower() in ('.xlsx', '.csv') else document_engine.read(_fp)
            else: _ft = ''
            memory_engine.record(_cid, 'text', _raw, _fp)
            mem = memory_engine.recall(_cid, _raw)
            f_ctx = file_registry.recall(_raw, _cid)
            text = (mem + '\n' + f_ctx + '\n' + _ft + '\n' + str(text))[:16000]
            # === AREAL WORKER BUS END ===
            if not text:
        ...
        logger.exception("task=%s intake error", task_id)
        await _fail_task(task_id, str(exc), "task_worker", chat_id, topic_id)
    finally:
